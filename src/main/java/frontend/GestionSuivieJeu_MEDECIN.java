...
public class GestionSuivieJeu_MEDECIN extends Application{
    ...
    private Button createNavButton(String text, String iconPath) {
        ...
        if (text.equals("Evaluation")) {
            button.setOnAction(e -> {
                ...
                GestionSuivieJeu_RATEMED RATEMED = new GestionSuivieJeu_RATEMED();
                try {
                    RATEMED.start(EVALUATIONStage);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                ((Stage) button.getScene().getWindow()).close();
        ...
        if (text.equals("Demandes d'ajout")) {
            button.setOnAction(e -> {
                Stage demandeStage = new Stage();
                GestionSuivieJeu_ACCEPTERJEU ACCEPTjeuApp = new GestionSuivieJeu_ACCEPTERJEU();
                try {
                    ACCEPTjeuApp.start(demandeStage);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
                ((Stage) button.getScene().getWindow()).close();
